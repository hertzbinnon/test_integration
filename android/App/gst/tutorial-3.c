...
static void *app_function (void *userdata) {
  ...
  //data->pipeline = gst_parse_launch("ahcsrc ! autovideoconvert ! videoscale ! video/x-raw,width=352,height=188 ! autovideosink", &error);
  //data->pipeline = gst_parse_launch("ahcsrc device-orientation=90 ! queue !  autovideoconvert ! videoscale ! video/x-raw,width=352,height=188 ! queue ! autovideosink openslessrc ! audioconvert ! audioresample ! autoaudiosink", &error);
  data->pipeline = gst_parse_launch("ahcsrc device-orientation=0 ! queue ! videoflip video-direction=90r !  autovideoconvert ! queue ! glimagesink openslessrc ! audioconvert ! audioresample ! openslessink", &error);
  if (error) {
