...
def parser_common(args,xml):
	context = ET.iterparse(StringIO(xml),events=('start','end'))
...
def parser_encode(xml):
		args_list = {'mode':'none','action':'encode','type':'none',
	...
	#try:
		...
		for src in sources:
			...
			for sout in souts:
				...
				print 'start =========================='
				output_param.append({'access_iface':sout['sendby'],'dst':sout['output']})
...
def parser(xml):
	...
	try:
		...
		if action == 'heartbeat':
			...
			error ,args_list = parser_encode(xml)	
		elif action == 'reset':
			error ,args_list = parser_encode(xml)	
			...
			error ,args_list = parser_config(xml)	
		else:
...
def wsgi_main():
	task_queue = Queue()
	thread.start_new_thread(start_wsgi, (eventlet.listen(('', 8090)),application,task_queue))
	command=() 
	while 1:
		try:
