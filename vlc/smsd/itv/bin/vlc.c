...
int main( int i_argc, char *ppsz_argv[] )
{
     //
     		int my_argc=0;
		char * my_argv[i_argc + 1];
     		for(int i = 0 ; i < i_argc; i++){
			my_argv[i] = (char*) malloc( strlen(ppsz_argv[i])+1 );
			memcpy(my_argv[i], ppsz_argv[i], strlen(ppsz_argv[i])+1);
			my_argc++;
			if(i != 0){
			 int j = strlen(ppsz_argv[i]);
			       for (j = j - 1; j >= 0; j--)
			      { ppsz_argv[i][j] = '\0';
			      }
			}
	...
	fprintf( stderr, "Please run  with not root \n");
        return 1;
    }
#endif
#endif
    ...
    if (isatty (STDERR_FILENO))
       ...
       /* fprintf (stderr, "VLC media player %s (revision %s)\n",
    	...
    	fprintf( stderr, "Streaming Media Server \n" );
    ...
    libvlc_instance_t *vlc = libvlc_new (my_argc, my_argv);
    if (vlc == NULL)
        return 1;
