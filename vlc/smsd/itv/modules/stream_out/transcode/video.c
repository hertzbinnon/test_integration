                                    ...
                                    block_t *in, block_t **out )
{
    ...
    *out = NULL;
    ...
    {
#ifdef Debug_Flag
			if(++id->dec_count % 2500 == 0)
				printf("[id: %x] dec_count %d, enc_count %d diff %d\n",id,id->dec_count,id->enc_count,id->dec_count-id->enc_count);
#endif
#ifndef Debug_Flag
	     char chroma_in[5]={0,0,0,0,0}, chroma_out[5];
	    vlc_fourcc_to_char(p_pic->format.i_chroma, chroma_in);
	    printf("chroma_in %s width %d, height %d, x %d, y %d, v_width %d, v_height %d, pixel_size %d bits, sar_num %d, sar_den %d\n", chroma_in, p_pic->format.i_width, p_pic->format.i_height, p_pic->format.i_x_offset, p_pic->format.i_y_offset, p_pic->format.i_visible_width, p_pic->format.i_visible_height, p_pic->format.i_bits_per_pixel, p_pic->format.i_sar_num, p_pic->format.i_sar_den);
	    for(int i = 0; i<p_pic->i_planes; i++)
	    	printf("xxxxxxxxxx: planes line number %d bytes of line %d\n", p_pic->p[i].i_lines,p_pic->p[i].i_pitch);
#endif
        if( unlikely (
             id->p_encoder->p_module &&
    ...
    if( p_sys->i_threads >= 1 )
    {
        ...
        *out = p_sys->p_buffers;
		...
		for(block_t* b=*out; b; b=b->p_next){
			id->enc_count++;
		}
#endif
    }
