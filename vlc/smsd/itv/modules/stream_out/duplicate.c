...
static bool ESSelected1( const es_format_t *fmt, char *psz_select )
{
    char  *psz_dup;
    char  *psz;
...
static bool ESSelected( const es_format_t *fmt, char *psz_select )
{
    ...
    printf("--------------- i_cat(%d) = %d,i_es(%d) = %d,i_prgm(%d) = %d\n",i_cat_flag,i_cat,i_es_flag,i_es,i_prgm_flag,i_prgm); //hb++
    if( i_cat == 1 || i_es == 1 || i_prgm == 1 )
    {
	    if(i_es_flag == 1 && i_prgm_flag == 1){
		   if(i_prgm == 1 && i_es == 1)
		    return true;
		   if(i_prgm == 1 && i_es == -1)
