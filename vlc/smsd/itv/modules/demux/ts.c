...
vlc_module_begin ()
    ...
    add_string( "ts-prog-location", NULL, "program-list-filename", "pat list", true )//hb++
...
struct demux_sys_t
{
    ...
    int		file_desc;// hb++ recording PAT and  PMT 
};
...
static int Open( vlc_object_t *p_this )
{
    ...
    //p_sys->i_ts_read = 50; //hb--
    p_sys->i_ts_read = 100; // hb++
    p_sys->csa = NULL;
...
# undef VLC_DVBPSI_DEMUX_TABLE_INIT
	...
	p_sys->i_programs = var_InheritInteger(p_demux, "program");//hb++
        p_sys->timeout = 0;//hb++
	vlc_sem_init (&p_sys->ready, 0);
		
    if( vlc_clone( &p_sys->timeout_thread, ThreadTimeout, p_demux, //hb++
                           VLC_THREAD_PRIORITY_HIGHEST ) )
    {
        msg_Err( p_demux, "cannot spawn sout access timeout thread" );
        return VLC_EGENERIC;
    }
    psz_string = var_CreateGetString( p_demux, "ts-prog-location" );//hb++
    p_sys->file_desc = -1;
    if (psz_string && *psz_string){
		p_sys->file_desc = open(psz_string,O_RDWR | O_CREAT |  O_SYNC | O_TRUNC,0777);
        	free( psz_string );
		if ( p_sys->file_desc < 0){
        		msg_Err( p_demux, "cannot open program.list file" );
			return VLC_EGENERIC;
		}
    }
    return VLC_SUCCESS;
...
/*
	...
	if(p_pid->i_flags == 0xff) { // hb++
                msg_Dbg( p_demux, "DVB Input error: ss" );
		return VLC_DEMUXER_EOF;//hb++
	}
*/
        if( !SEEN(*p_pid) )
        {
...
*/
		...
		/*if(p_pid->i_flags == 0xff) {// hb++
                	msg_Dbg( p_demux, "DVB Input error: cc" );
			return VLC_DEMUXER_EOF;
		}//hb++*/
            break;
...
void  delayTest(demux_t* p_demux)
{
    for( int i=0; i< p_demux->p_sys->pid[0].u.p_pat->programs.i_size; i++ )
    {
        ts_pmt_t *p_pmt = p_demux->p_sys->pid[0].u.p_pat->programs.p_elems[i]->u.p_pmt;
#define  DELAY_TIME 0.1
        double video_time ,audio_time[32];
        //static double max_delay_audio[32];
        int audio_track = 0;
        for( int j=0; j < p_pmt->e_streams.i_size; j++ )
        {
            ts_pid_t *espid = p_pmt->e_streams.p_elems[j];
            if(espid->u.p_pes->es.fmt.i_cat == 1){
              video_time =  espid->i_elapsed_dts/90000.0;
              //printf("video %f\n",video_time);
            }else if(espid->u.p_pes->es.fmt.i_cat == 2){
              audio_time[audio_track++] =  espid->i_elapsed_pts/90000.0;
              //printf("audio %f\n",audio_time[audio_track-1]);
            }
        }
        for(int i=0; i < audio_track; i++){
                double delayed_time = video_time - audio_time[i];
                double sync_time = 0;
                //printf("audio delay relative %f\n",delayed_time);
                //printf("max audio delay relative %f\n",max_delay_audio[i]<delayed_time? max_delay_audio[i]=delayed_time:max_delay_audio[i]);
                if( delayed_time > DELAY_TIME ){ // audio delayed
                   if(p_pmt->delayed_time == 0){
                      p_pmt->delayed_time = DELAY_TIME;
                        sync_time = -DELAY_TIME ;
                        //var_SetInteger( p_demux->p_input, "audio-delay",  sync_time * 1000000);
                        msg_Err(p_demux,"video delayed %f\n",sync_time);
                   }else if(p_pmt->delayed_time < delayed_time){
                        sync_time = p_pmt->delayed_time - delayed_time;
                        //var_SetInteger( p_demux->p_input, "audio-delay",  sync_time * 1000000);
                        p_pmt->delayed_time = delayed_time;
                        msg_Err(p_demux,"video delay %f\n",sync_time);
                   }
                }else if (delayed_time < -DELAY_TIME){ // audio hastened
                   if(p_pmt->delayed_time == 0){
                        p_pmt->delayed_time = -DELAY_TIME ;
                        sync_time = DELAY_TIME ;
                        //var_SetInteger( p_demux->p_input, "audio-delay",  sync_time * 1000000);
                        msg_Err(p_demux,"audio delay %f\n",sync_time);
                   }else if(p_pmt->delayed_time > delayed_time){
                        sync_time = p_pmt->delayed_time - delayed_time;
                        //var_SetInteger( p_demux->p_input, "audio-delay",  sync_time * 1000000);
                        p_pmt->delayed_time = delayed_time;
...
static void clockRecovery( demux_t* p_demux,ts_pid_t *pid, mtime_t* pts, mtime_t* dts){
	mtime_t i_pts = *pts;	
	mtime_t i_dts = *dts;	

        int fmt_cat = pid->u.p_pes->es.fmt.i_cat;

        mtime_t refs_time = pid->i_first_pts;
       if(i_pts == -1 && i_dts == -1){
		msg_Err(p_demux,"%x  ERROR PID = %d -> pts %"PRId64" , dts %"PRId64" \n", pid, pid->i_pid,i_pts,i_dts );
           	return ;
       }else if(i_dts == -1 && i_pts > 0){
           i_dts = i_pts;
       }
        if( !refs_time) pid->i_first_pts = i_pts;
        pid->i_elapsed_pts = i_pts - pid->i_first_pts;

        refs_time = pid->i_first_dts;
        if( !refs_time) pid->i_first_dts = i_dts;
        pid->i_elapsed_dts = i_dts - pid->i_first_dts;

#if 0
	if(i_pts != -1){
		if(!pid->i_last_pts) pid->i_last_pts = i_pts;
		pid->i_current_pts = i_pts;
		pid->i_delta_pts = pid->i_last_pts - pid->i_current_pts - pid->i_drift_pts;
		if(pid->i_delta_pts > 5400000 || pid->i_delta_pts < -5400000){
			msg_Err(p_demux,"%x  ERROR PID = %d -> Pts %"PRId64" , last %"PRId64" \n", pid, pid->i_pid,i_pts, pid->i_last_pts);
			pid->i_drift_pts += pid->i_delta_pts;	
			pid->i_delta_pts = 0;
			msg_Err(p_demux,"%x  ERROR PID = %d -> Pts %"PRId64" , last %"PRId64" drift = %"PRId64"\n", pid, pid->i_pid,i_pts, pid->i_last_pts,pid->i_drift_pts);
		}
		pid->i_current_pts += pid->i_drift_pts;
		//msg_Info(p_demux,"%x  ERROR PID = %d -> last_Pts =  %"PRId64" , cur_Pts = %"PRId64", sys_Pts = %"PRId64", diff = %"PRId64",",
		//			pid, pid->i_pid,pid->i_last_pts,i_pts,pid->i_current_pts,pid->i_current_pts - pid->i_last_pts);
		pid->i_last_pts = pts = pid->i_current_pts;
	}
	if(i_dts != -1){
		if(!pid->i_last_dts) pid->i_last_dts = i_dts;
		pid->i_current_dts = i_dts;
		pid->i_delta_dts = pid->i_last_dts - pid->i_current_dts - pid->i_drift_dts;;
		if(pid->i_delta_dts > 5400000){
			msg_Err(p_demux,"%x  ERROR PID = %d -> Dts %"PRId64" , last %"PRId64" \n", pid, pid->i_pid,i_dts, pid->i_last_dts);
			pid->i_drift_dts += pid->i_delta_dts;	
			pid->i_delta_dts = 0;
		}
		pid->i_current_dts += pid->i_drift_dts;
		//msg_Dbg(p_demux,"%x  ERROR PID = %d -> last_Dts =  %"PRId64" , cur_Dts = %"PRId64", sys_Dts = %"PRId64", diff = %"PRId64",",
		//			pid, pid->i_pid,pid->i_last_dts,i_dts,pid->i_current_dts,pid->i_current_dts - pid->i_last_dts);
		pid->i_last_dts = dts = pid->i_current_dts;
	}
#endif	
}
#undef lipsync 
...
#ifdef lipsync
	clockRecovery(p_demux,pid,&i_pts,&i_dts);
#endif
...
//	printf("%x  ERROR Pts %"PRId64 " , Dts %"PRId64" \n",pid->i_pid,  i_pts,i_dts);
		...
		p_demux->p_sys->timeout = 0;//hb++
#ifdef lipsync
		delayTest(p_demux);//hb++
#endif
            }
            else
            {
...
static void ProgramSetPCR( demux_t *p_demux, ts_pmt_t *p_pmt, mtime_t i_pcr )
{
    ...
    {
	//hb++
	//if(p_pmt->i_last_pcr && (i_pcr - p_pmt->i_last_pcr > 5400000 || i_pcr - p_pmt->i_last_pcr < -5400000)) {
	//msg_Err( p_demux,"pmt_number = %d last_pcr = %"PRId64" cur_pcr =%"PRId64" sys_pcr = %"PRId64" diff_pcr = %"PRId64"\n",p_pmt->i_number, p_pmt->i_last_pcr, i_pcr,VLC_TS_0 + i_pcr * 100 / 9 ,i_pcr - p_pmt->i_last_pcr );
	//	es_out_Control(p_demux->out, ES_OUT_RESET_PCR);
	//	p_pmt->i_last_pcr = 0;
	//	exit(199);
	//}else{
	//	p_pmt->i_last_pcr = i_pcr;
	//
...
static bool GatherData( demux_t *p_demux, ts_pid_t *pid, block_t *p_bk )
{
	...
	p_demux->p_sys->i_cc_checking = 0;//hb++
	input_State(p_demux->p_input,PLAYING_S);
    }
    else
    {
        if( pid->i_cc == 0xff )
        {
            msg_Warn( p_demux, "first packet for pid=%d cc=0x%x",
                      pid->i_pid, i_cc );
            pid->i_cc = i_cc;
		p_demux->p_sys->i_cc_checking = 1;//hb++
        }
		...
		input_State(p_demux->p_input,ERROR_S_I);
		p_demux->p_sys->i_cc_checking++ ;//hb++
		if(p_demux->p_sys->i_cc_checking>10) {// hb++
			pid->i_flags = 0xff;
			return false;
		}
            	pid->i_cc = i_cc;
	    	if(!b_unit_start){
			msg_Err(p_demux,"this packet was discard");
            		block_Release( p_bk ); // hb++
	    		return true; //hb++
	    	}
                              ...
                              const dvbpsi_pmt_es_t *p_dvbpsies )
{
    ...
    if( p_es->fmt.psz_language )
    {
        ...
        msg_Dbg( p_demux, "found language: %s", p_es->fmt.psz_language);
                ...
                if(p_demux->p_sys->file_desc > 0){//hb++
                        char PMT[256];
                        memset(PMT,0,sizeof(PMT));
                        sprintf(PMT," language=%s",p_es->fmt.psz_language);
                        if(write(p_demux->p_sys->file_desc,PMT,strlen(PMT)) != strlen(PMT)){
                                msg_Err(p_demux,"write error to program.list ");
                        }
                }
...
static void PMTCallBack( void *data, dvbpsi_pmt_t *p_dvbpsipmt )
{
	...
	char PMT[256];
	if(p_sys->file_desc > 0){//hb++
		memset(PMT,0,sizeof(PMT));
		sprintf(PMT,"PMT program_number=%d pid=%d\n",p_dvbpsipmt->i_program_number, pmtpid->i_pid);
		if(write(p_sys->file_desc,PMT,strlen(PMT)) != strlen(PMT)){
			msg_Err(p_demux,"write error to program.list ");
		}
	}
...
static void PATCallBack( void *data, dvbpsi_pat_t *p_dvbpsipat )
{
	...
	#if 0 //hb++
        char pat[100];
        char* psz_string = var_CreateGetString( p_demux, "ts-prog-location" );
        char location[256];
        memcpy(location,psz_string,strlen(psz_string)+1);
        sprintf(pat,"cat /dev/null > %s",location);
        system(pat);
        free( psz_string );
	#endif//hb++
         ...
         p_program = p_program->p_next )
    {
        ...
        sprintf(pat,"echo 'program_number=%d program_pid=%d' >> %s",p_program->i_number,p_program->i_pid,location);
        system(pat);
	#endif//hb++
        ...
        if(p_sys->i_programs == p_program->i_number &&  (p_sys->programs.i_size == 0 ||
            ProgramIsSelected( p_sys, p_program->i_number )) )
        {
            if( p_sys->programs.i_size == 0 )
            {
...
static void _ProgramSetPCR( demux_t *p_demux, ts_pmt_t *p_pmt, mtime_t i_pcr )
{
    demux_sys_t *p_sys = p_demux->p_sys;
	...
	p_pmt->i_delta = p_pmt->i_last_pcr -  p_pmt->pcr.i_current - p_pmt->i_drift;

	if(p_pmt->i_delta > 5400000 || p_pmt->i_delta < -5400000){
		msg_Err( p_demux,"pmt_number = %d last_pcr = %"PRId64" cur_pcr =%"PRId64" sys_pcr = %"PRId64" diff_pcr = %"PRId64"\n",p_pmt->i_number, p_pmt->i_last_pcr, i_pcr,p_pmt->pcr.i_current,i_pcr - p_pmt->i_last_pcr );
		input_State(p_demux->p_input,ERROR_S_N);
		p_pmt->i_drift += p_pmt->i_delta ;
		p_pmt->i_delta= 0;
		input_State(p_demux->p_input,PLAYING_S);
	}
	
	p_pmt->pcr.i_current += p_pmt->i_drift;
    if ( p_sys->i_pmt_es )
    {
		//msg_Info( p_demux,"pmt_number = %d last_pcr = %"PRId64" cur_pcr =%"PRId64" sys_pcr = %"PRId64" diff_pcr = %"PRId64"",p_pmt->i_number, p_pmt->i_last_pcr,i_pcr, p_pmt->pcr.i_current,p_pmt->pcr.i_current - p_pmt->i_last_pcr );
        es_out_Control( p_demux->out, ES_OUT_SET_GROUP_PCR,
                        p_pmt->i_number, VLC_TS_0 + p_pmt->pcr.i_current * 100 / 9 );
				p_pmt->i_last_pcr  = p_pmt->pcr.i_current;
    }
...
#define MIN(x,y) (int64_t)(((x)<(y)) ? x : y)
	...
	int count = 0;
    int64_t network_caching = INT64_C(1000) *  var_InheritInteger(p_demux, "network-caching");
	int64_t to = network_caching + 500000;
	for(vlc_object_t* o=p_demux; o; o=o->p_parent){
	...
	msg_Warn(p_demux,"Timeout started ");
	while(1){
		//msg_Dbg(p_demux, "Timeout %lld buffering state %d count %d\n", to, stream_output,count);
		//usleep( 8500000 );	
		usleep( 5500000 );	
		if( !p_sys->timeout ){
			p_sys->timeout = 1;
			 msg_Dbg(p_demux, "Demuxer is active  \n");
			//count = 0;
		}else{
			/*if(stream_output->i_flags > 1) { // input is playing
         	...
         	msg_Err( p_demux, "No input source availible after waiting ");
		//exit(253);
		exit(199);
		}
