 ...
 * $Id: e5c07f4c45191d917aba65b2c4aa5e9428a7a8e3 $
 *
...
static void* ThreadRead_avconv( void *data );
...
static int Open( vlc_object_t *p_this )
{
    ...
    if( vlc_clone( &sys->thread, ThreadRead_avconv, p_access,
...
error:
    ...
    if( vlc_clone( &sys->timeout_thread, ThreadTimeout, p_access, //hb++
                           VLC_THREAD_PRIORITY_HIGHEST ) )
    {
        msg_Err( p_access, "cannot spawn access timeout thread" );
        return VLC_EGENERIC;
    }
    return VLC_SUCCESS;
}
...
static void Close( vlc_object_t *p_this )
{
    ...
    vlc_cancel( sys->timeout_thread ); //hb++
    vlc_join( sys->timeout_thread, NULL ); //hb++
    block_FifoRelease( sys->fifo );
    //net_Close( sys->fd );
    free( sys );
}
...
static void* ThreadTimeout( void *data ){//hb++
	...
	msg_Warn(p_access,"Access Timeout started ");
	while(1){
		if( !p_sys->timeout ){
			p_sys->timeout = 1;
		}else{
			msg_Err(p_access,"No udp package received");
	   		exit(197);
		}
		sleep(100000);	
	}
...
int avconv_main(int, char** );
static void* ThreadRead_avconv( void *data )
{
    access_t *access = data;
    access_sys_t *sys = access->p_sys;
    char url_string[256]={0};
    sprintf(url_string,"udp://%s",access->psz_location);
    int64_t fifo_num = (int64_t)sys->fifo,fifo_size=sys->fifo_size;
    char fifo_num_string[64];
    char fifo_size_string[64];
    sprintf(fifo_num_string,"%lld",fifo_num);
    sprintf(fifo_size_string,"%lld",fifo_size);
    char *argv[]={"avconv","-v","debug","-i",url_string,"-c","copy","-fifo_num",fifo_num_string,"-fifo_size",fifo_size_string,"-f","mpegts","vlcfifo:1"};
    int argc = sizeof(argv)/sizeof(argv[0]);

    int exit_code = 0;
    printf("start avconv ====> %lld\n",sys->fifo);
    for(int i=0;i<argc;i++)
	printf("%s ",argv[i]);
    printf("\nstart avconv <===\n");
    exit_code = avconv_main(argc,argv);
