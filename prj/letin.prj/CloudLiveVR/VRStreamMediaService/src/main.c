...
static GstElement *bin1, *bin2, *bin3, *bin4, *bin5, *bin6, *bin7, *bin8;
...
static gboolean perform_step (gpointer pstep)
{
  ...
  switch (step) {
    case 0:
      ...
      sprintf(pattern_str,"( uridecodebin uri=%s ! compositor  ! videoconvert ! timeoverlay ! queue ! xvimagesink name=v4llive%d )", "http://192.168.0.134:8080/daobo4.ts", numbs++);
