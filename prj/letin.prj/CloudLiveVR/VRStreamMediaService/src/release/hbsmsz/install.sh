...
sudo cp -rf vrsmsz/Python-3.7.5/* /usr/local/
...
#cd $INSTALL_PATH
cp -rf vrsmsz/django-master /var/local/vrsmsz/env
cp -rf vrsmsz/uploadmodule /var/local/vrsmsz/env
...
sudo cp -rf linux_x86_64/lib/* /usr/local/lib64
sudo cp -rf  vrsmsz/nginx /usr/local/
sudo chown -R $USER.$USER /usr/local/nginx
sudo mkdir /var/log/vrsmsz /var/run/vrsmsz
rm -rf $INSTALL_PATH/
