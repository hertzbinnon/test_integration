...
static int DecoderPlaySout( decoder_t *p_dec, block_t *p_sout_block )
{
    ...
    {
		msg_Dbg(p_dec,"release %s\n", __func__);
        block_Release( p_sout_block );
        // return VLC_EGENERIC; // hb--
    }
}
...
static void *DecoderThread( void *p_data )
{
	...
	p_owner->timeout++;//hb++
        ...
        while( vlc_fifo_IsEmpty( p_owner->p_fifo ) )
        {
            ...
            p_owner->b_idle = true;
			//msg_Dbg(p_dec,"waiting data to decode (type = %d)",p_dec->fmt_in.i_codec);//hb++
            vlc_fifo_Wait( p_owner->p_fifo );
			//msg_Dbg(p_dec,"receive data to decode (type = %d)",p_dec->fmt_in.i_codec);//hb++
...
#ifdef Debug_Flag
    p_owner->timeout = 0;//hb++
    if( vlc_clone( &p_owner->timeout_thread, ThreadTimeout, p_owner, //hb++
                           VLC_THREAD_PRIORITY_HIGHEST ) )
    {
        printf( "cannot spawn sout access timeout thread" );
        return VLC_EGENERIC;
    }
#endif
    return p_dec;
...
void input_DecoderDecode( decoder_t *p_dec, block_t *p_block, bool b_do_pace )
{
	...
	if(vlc_fifo_GetBytes(p_owner->p_fifo) > 40*1024*1024) // hb++
		msg_Warn(p_dec,"[%d] fifo size %d (%d)\n", syscall(__NR_gettid), vlc_fifo_GetBytes(p_owner->p_fifo),b_do_pace);
    vlc_fifo_Lock( p_owner->p_fifo );
    if( !b_do_pace )
    {
        ...
        if( vlc_fifo_GetBytes( p_owner->p_fifo ) > 40*1024*1024 )
        {
...
#ifdef Debug_Flag
static void* ThreadTimeout( void *data ){//hb++
    decoder_owner_sys_t *p_owner = data;
	while(1){
		sleep(3);
		if(p_owner->timeout!=0)
			printf("count = %d\n",p_owner->timeout);
		p_owner->timeout = 0;
	}	
