...
#undef input_State
void input_State( input_thread_t *p_input, int i_state )
{
...
static void MainLoop( input_thread_t *p_input, bool b_interactive )
{
	...
	/*if( p_input->p->p_sout->i_flags == 0xff ){ //hb++
		//p_input->b_error = true;
		input_ChangeState( p_input, ERROR_S);
 	}*/
		if(es_out_GetBuffering( p_input->p->p_es_out ))
			p_input->p->p_sout->i_flags = 1; // buffering? hb++
		else{
			p_input->p->p_sout->i_flags= 10;// no input?
		}
    }
