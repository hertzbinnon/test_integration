...
static picture_t *Filter( filter_t *p_filter, picture_t *p_picture)
{
    ...
    {
        msg_Err(p_filter,"Duplicating pictures next_pts = %"PRId64" interval = %"PRId64" current = %"PRId64" diff = %"PRId64"\n",date_Get( &p_sys->next_output_pts ) , p_sys->i_output_frame_interval  , p_picture->date,(date_Get( &p_sys->next_output_pts ) + p_sys->i_output_frame_interval ) - p_picture->date );//hb++
        ...
        last_pic = p_tmp;
        date_Increment( &p_sys->next_output_pts, p_filter->fmt_out.video.i_frame_rate_base );
	p_picture->date = date_Get( &p_sys->next_output_pts ) + p_sys->i_output_frame_interval;//hb++ 
    }
