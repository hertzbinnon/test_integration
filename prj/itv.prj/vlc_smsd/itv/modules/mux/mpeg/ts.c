...
static int Open( vlc_object_t *p_this )
{
    ...
    */
	...
	p_sys->i_tt_checking = 0; //hb++
	p_sys->i_tt_count= -1; //hb++
	p_sys->i_tt_istime= -1; //hb++

	p_sys->timeout = 0;//hb++
	vlc_sem_init (&p_sys->ready, 0);
		
    if( vlc_clone( &p_sys->timeout_thread, ThreadTimeout, p_mux, //hb++
                           VLC_THREAD_PRIORITY_HIGHEST ) )
    {
        msg_Err( p_mux, "cannot spawn sout access timeout thread" );
        return VLC_EGENERIC;
    }
    return VLC_SUCCESS;
}
...
static void DelStream( sout_mux_t *p_mux, sout_input_t *p_input )
{
    ...
    if( p_sys->i_pcr_pid == p_stream->ts.i_pid )
    {
        ...
        //p_sys->i_pcr_pid = 0x1fff; //hb--
        //p_sys->p_pcr_input = NULL; //hb--
        for (int i = 0; i < p_mux->i_nb_inputs; i++ )
        {
...
static bool MuxStreams(sout_mux_t *p_mux )
{
	...
	p_sys->timeout = 0;//hb++
#if 0
    	if (p_sys->i_tt_checking > 30 ){//hb++
		msg_Err( p_mux, "DVB Input tt Error");
		p_sys->i_tt_checking =0;
		exit(3);
		//DelStream(p_mux, p_sys->p_pcr_input);
		//AddStream(p_mux, p_sys->p_pcr_input);
		return true;
	}else if (p_sys->i_tt_count > 0){
		p_sys->i_tt_checking++;
		p_sys->i_tt_count = 0 ;
        	msg_Warn( p_mux, "i_tt_checking = %d", p_sys->i_tt_checking);// hb++
	}else if (p_sys->i_tt_count == 0 && p_sys->i_tt_checking > 100 ){
        	msg_Warn( p_mux, "dis_tt_checking = %d", p_sys->i_tt_checking);// hb++
		p_sys->i_tt_checking =0;
	}
#endif
...
// hertz padding
    //msg_Dbg( p_mux, "pck=%d", i_packet_count );
    if ( i_pcr_length / 1000 > 0 && p_sys->i_bitrate_min > 0)
    {
        int i_bitrate = ((uint64_t)i_packet_count * 188 * 8000)
                          / (uint64_t)(i_pcr_length / 1000);
        if ( p_sys->i_bitrate_min && p_sys->i_bitrate_min > i_bitrate )
        {

            int  i_diff_bitrate  = p_sys->i_bitrate_min - i_bitrate;
            int  i_packet_padding = i_diff_bitrate / 8 / 188 * (i_pcr_length * 1.0  / 1000000.0);
            for(int i=0; i < i_packet_padding; i++){
                  block_t *p_ts = block_Alloc( 188 );
                  p_ts->p_buffer[0] = 0x47;
                  p_ts->p_buffer[1] = 0x1f;
                  p_ts->p_buffer[2] = 0xff;
                  BufferChainAppend( p_chain_ts, p_ts );
            }
            msg_Warn( p_mux, "min estimated bitrate exceeded at %"PRId64
                      " (%d bi/s for %d pkt in %"PRId64" us), padding + %d",
                      i_pcr_dts + p_sys->i_shaping_delay * 3 / 2 - mdate(),
                      i_bitrate, i_packet_count, i_pcr_length,i_packet_padding );
            i_packet_count = p_chain_ts->i_depth;
            //msg_Dbg( p_mux, "--> pck=%d", i_packet_count );
        }
    }
    //msg_Dbg( p_mux, "pck = %d", i_packet_count );
...
#define MIN(x,y) (int64_t)(((x)<(y)) ? x : y)
	...
	int count = 0;
    int64_t network_caching = INT64_C(1000) *  var_InheritInteger(p_mux, "network-caching");
	int64_t to = network_caching + 500000;
	for(vlc_object_t* o=p_mux; o; o=o->p_parent){
		//printf("%s\n",o->psz_object_type);
	...
	msg_Warn(p_mux,"Timeout started ");
	while(1){
		...
		if( !p_sys->timeout ){
			...
			/*if(stream_output->i_flags > 1) { // input is playing
         	...
         	msg_Err( p_mux, "No transcoding output availible ");
		exit(253);
		}
		if( p_mux->i_nb_inputs < 2){
         		msg_Err( p_mux, "Only one stream on program: %s",p_mux->pp_inputs[0]->fmt.i_cat==AUDIO_ES?"audio":"video");
			exit(252);
		}
