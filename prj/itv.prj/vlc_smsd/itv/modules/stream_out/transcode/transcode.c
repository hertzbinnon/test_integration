...
static void* SendThread( void *data );//hb++
static int SendFifo( sout_stream_t *p_stream, sout_stream_id_sys_t *id,
                 block_t *p_buffer ); //hb++
/*****************************************************************************
 * Open:
 *****************************************************************************/
static int Open( vlc_object_t *p_this )
{
    ...
    p_sys->p_fifo = block_FifoNew();//hb++
    if( vlc_clone( &p_sys->timeout_thread, SendThread, p_sys, //hb++
...
typedef struct command_t command_t; // hb++
struct command_t{
...
static int SendFifo( sout_stream_t *p_stream, sout_stream_id_sys_t *id, //hb++
                 block_t *p_buffer )
{
    sout_stream_sys_t *p_sys = p_stream->p_sys;
	command_t *p_cmd;
	block_t *p_block;
	p_block = block_Alloc(sizeof(*p_cmd));

	p_cmd = (command_t *)p_block->p_buffer;
	p_cmd->p_stream = p_stream;
	p_cmd->id= id;
	p_cmd->p_buffer= p_buffer;
...
static void* SendThread( void *data ){//hb++
	...
	while(1){
		...
		cmd = *((command_t *)p_block->p_buffer);
		Send(cmd.p_stream, cmd.id, cmd.p_buffer);
