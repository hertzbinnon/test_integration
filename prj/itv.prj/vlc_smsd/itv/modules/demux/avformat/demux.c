...
static int Demux( demux_t *p_demux )
{
	...
	p_frame=NULL;
	    ...
	    if(pkt.data[4] == 0x67){ //hb++
	    	for(int i=0; i<pkt.size-5; i++){
	    		if(pkt.data[i]==0 &&pkt.data[i+1]==0 &&pkt.data[i+2]==0 &&pkt.data[i+3]==1 && pkt.data[i+4]==0x65 ){

				p_frame = block_Alloc(pkt.size - i);
				memcpy(p_frame->p_buffer, pkt.data+i, pkt.size - i);
				int frame_size = pkt.size - i;
				printf("^^^^^^^^^^^^^^^^^  %x %x %x %x \n",  p_frame->p_buffer[0], p_frame->p_buffer[1],  p_frame->p_buffer[2],  p_frame->p_buffer[3] );
				p_frame->p_buffer[3] = frame_size & 0xff;
				p_frame->p_buffer[2] = frame_size  >> 8  & 0xff;
				p_frame->p_buffer[1] = frame_size  >> 16 & 0xff;
				p_frame->p_buffer[0] = frame_size  >> 24 & 0xff;
				printf("%x %x %x %x \n",  p_frame->p_buffer[0], p_frame->p_buffer[1],  p_frame->p_buffer[2],  p_frame->p_buffer[3] );
			}
	    	}
	    }/*else {
...
//#define AVFORMAT_DEBUG // hb++
#ifdef AVFORMAT_DEBUG
    msg_Dbg( p_demux, "%x %x %x %x %x %x" , p_frame->p_buffer[0], p_frame->p_buffer[1],p_frame->p_buffer[2],p_frame->p_buffer[3],p_frame->p_buffer[4]);
    msg_Dbg( p_demux, "tk[%d] pkt.dts =%"PRId64" pkt.pts=%"PRId64 " avg_fps=%d",
             	pkt.stream_index, pkt.dts, pkt.pts,p_stream->avg_frame_rate.num );
