...
class EventType(_Enum):
    ...
    _enum_names_ = {
        ...
        2001: 'MediaPlayerEncounteredError_I',
        2002: 'MediaPlayerEncounteredError_N',
        2003: 'MediaPlayerEncounteredError_T',
    }
...
EventType.MediaPlayerEncounteredError_I = EventType(2001)
EventType.MediaPlayerEncounteredError_N = EventType(2002)
EventType.MediaPlayerEncounteredError_T = EventType(2003)
...
State.Error_I        = State(8)
State.Error_N        = State(9)
State.Error_T        = State(10)
...
discarded.
    ''' 
    MediaReadCb = ctypes.CFUNCTYPE(ctypes.POINTER(ctypes.c_size_t), ctypes.c_void_p, ctypes.c_char_p, ctypes.c_size_t)
...
if __name__ == '__main__':
    ...
    if sys.argv[1:] and sys.argv[1] not in ('-h', '--help'):
    	...
    	def err_callback(event):
        	print('error of media stream (event %s)' % event.type)
        	#sys.exit(0)
	...
	"""
	args_string = "-vv --miface eth1 udp://@239.100.198.100:1234  --avcodec-fast  --avcodec-threads 24 --sout=#udp{mux=ts,dst=192.168.8.115:5555}"
	args = args_string.split(' ')
	vlc_int = Instance(args_string)
	media_player = vlc_int.media_player_new()
        media = vlc_int.media_new('udp://@239.100.198.100:1234')
        media_player.set_media(media)

	media_player.play()
	"""
	args_string = "-v --miface eth1 udp://@:12345  --avcodec-fast  --avcodec-threads 24 --sout=#udp{mux=ts,dst=192.168.8.115:5556}"
	args = args_string.split(' ')
	vlc_int = Instance(args_string)
	media_player = vlc_int.media_player_new()
        media = vlc_int.media_new('udp://@:12345')
        media_player.set_media(media)

        event_manager = media_player.event_manager()
        event_manager.event_attach(EventType.MediaPlayerEncounteredError_I, err_callback)

	media_player.play()

	wait_playing(media_player)
