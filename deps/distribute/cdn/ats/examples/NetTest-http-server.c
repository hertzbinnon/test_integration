...
struct NetTesterSM : public Continuation {
  ...
  {
    Debug("net_test", "thread=0x%llx WRITE\n",(unsigned long long)pthread_self());
    switch (event) {
    case VC_EVENT_WRITE_READY:
      Debug("net_test", "WRITE_READY");
      break;
...
test_main()
{
  ...
  /*Action *a = */netProcessor.accept(new NetTesterAccept(new_ProxyMutex()), opt);
