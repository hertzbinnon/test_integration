...
static const AVOption vlcfifo_options[] = {
   { "fifo_size", "Truncate existing files on write", offsetof(FifoContext, fifo_size), AV_OPT_TYPE_INT64, { .i64 = 0 }, 0, INT64_MAX, AV_OPT_FLAG_ENCODING_PARAM },
   { "fifo_num", "Truncate existing files on write", offsetof(FifoContext, fifo_num), AV_OPT_TYPE_INT64, { .i64 = 0 }, 0, INT64_MAX, AV_OPT_FLAG_ENCODING_PARAM },
   { NULL }
};

static const AVClass vlcfifo_class = {
    .class_name = "vlcfifo",
    .item_name  = av_default_item_name,
    .option     = vlcfifo_options,
    .version    = LIBAVUTIL_VERSION_INT,
};

static int vlcfifo_read(URLContext *h, unsigned char *buf, int size)
{
    return 0;
}

static int vlcfifo_write(URLContext *h, const unsigned char *buf, int size)
{
    FifoContext *c = h->priv_data;
 ...
 av_log(h, AV_LOG_DEBUG,   "---> fifo put\n");
...
static int vlcfifo_open(URLContext *h, const char *uri, int flags, AVDictionary **opts)
{
    FifoContext *c = h->priv_data;
    int access;
    int fd;
...
// av_log(h, AV_LOG_DEBUG,"--++----------\n");
// av_log(h, AV_LOG_DEBUG,   "---> fifo_num = %lld\n",c->fifo_num);
// av_log(h, AV_LOG_DEBUG,   "---> fifo_addr = %lld\n",c->fifo);
// av_log(h, AV_LOG_DEBUG,   "---> fifo_size= %lld\n",c->fifo_size);
...
static int vlcfifo_close(URLContext *h)
{
    FifoContext *c = h->priv_data;
    return 0;
    //return close(c->fd);
...
URLProtocol ff_vlcfifo_protocol = {
    .name                = "vlcfifo",
    .url_open2            = vlcfifo_open,
//    .url_read            = vlcfifo_read,
    .url_write           = vlcfifo_write,
//    .url_seek            = vlcfifo_seek,
    .url_close           = vlcfifo_close,
//    .url_get_vlcfifo_handle = fifo_get_handle,
//    .url_check           = vlcfifo_check,
    .priv_data_size      = sizeof(FifoContext),
    .priv_data_class     = &vlcfifo_class,
};
